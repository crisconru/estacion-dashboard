...
class PyRemoto():
    ...
    def __init__(self, img_carpeta, iconos):
        ...
        # Título
        self.raiz.title("PyRemoto " + __version__)
        # Icono app
        ...
        # Deshabilita submenús flotantes
        self.raiz.option_add('*tearOff', False)
        # Maximiza ventana completa
        self.raiz.attributes('-fullscreen', True)
        # Establece tamaño minimo ventana
        ...
        menu1.add_command(label='Conectar...', command=self.f_conectar,
                          underline=0, accelerator="Ctrl+c", image=icono2,
                          compound=LEFT)
        menu1.add_separator()  # Agrega un separador
        menu1.add_command(label='Salir', command=self.f_salir, underline=0,
                          accelerator="Ctrl+q", image=icono3, compound=LEFT)
        # DEFINIR SUBMENÚ 'Opciones':
        self.menu2.add_checkbutton(label='Barra de Estado',
                                   variable=self.estado, onvalue=1,
                                   offvalue=0, command=self.f_verestado)
        self.menu2.add_separator()
        self.menu2.add_radiobutton(label='ssh', variable=self.CFG_TIPOCONEX,
                                   command=self.f_cambiaropc, value=1)
        self.menu2.add_radiobutton(label='sftp', variable=self.CFG_TIPOCONEX,
                                   command=self.f_cambiaropc, value=2)
        self.menu2.add_radiobutton(label='rdp', variable=self.CFG_TIPOCONEX,
                                   command=self.f_cambiaropc, value=3)
        self.menu2.add_separator()
        self.menu2.add_radiobutton(label='xterm', variable=self.CFG_TIPOEMUT,
                                   command=self.f_cambiaropc, value=1)
        self.menu2.add_radiobutton(label='uxterm', variable=self.CFG_TIPOEMUT,
                                   command=self.f_cambiaropc, value=2)
        self.menu2.add_radiobutton(label='xfce4-terminal',
                                   variable=self.CFG_TIPOEMUT,
                                   command=self.f_cambiaropc, value=3)
        self.menu2.add_separator()
        self.menu2.add_radiobutton(label='Thunar', variable=self.CFG_TIPOEXP,
                                   command=self.f_cambiaropc, value=1)
        self.menu2.add_radiobutton(label='Nautilus', variable=self.CFG_TIPOEXP,
                                   command=self.f_cambiaropc, value=2)
        self.menu2.add_separator()
        self.menu2.add_command(label='Guardar', command=self.f_opcionguardar,
        ...
        menu3.add_command(label="Acerca de", command=self.f_acerca,
        ...
        mensaje = f" {info1}: {info2} - {info3}"
        self.barraest = Label(self.raiz, text=mensaje, bd=1, relief=SUNKEN,
                              anchor=W)
    ...
    def f_acerca(self):
        ...
        etiq2 = Label(marco1, text=f"{PyRemoto }__version__",
...
def main():
    ...
    img_carpeta = f'{app_carpeta}{os.sep}imagen{os.sep}'
    # DECLARAR Y VERIFICAR ICONOS DE LA APLICACIÓN:
    iconos = (f"{img_carpeta}pyremoto64x64.png",
              f"{img_carpeta}conec16x16.png",
              f"{img_carpeta}salir16x16.png",
              f"{img_carpeta}star16x16.png",
              f"{img_carpeta}conec32x32.png",
              f"{img_carpeta}salir32x32.png")
    error1 = f_verificar_iconos(iconos)

    if not error1:
        PyRemoto(img_carpeta, iconos)
    return(0)
